...
function findNextQueuedDriver(pickupLat, pickupLng, excludeIds) {
  ensureDrivers();
  const drivers = getDrivers().filter(d => d.status === 'AVAILABLE' && d.zone && !excludeIds.includes(d.id));
  if (drivers.length === 0) return null;
  const zoneDist = {};
  ZONES.forEach(z => { zoneDist[z.id] = distanceMiles(pickupLat, pickupLng, z.lat, z.lng); });
  drivers.sort((a, b) => {
    const da = zoneDist[a.zone] != null ? zoneDist[a.zone] : Infinity;
    const db = zoneDist[b.zone] != null ? zoneDist[b.zone] : Infinity;
